...
\title{Computer Model Emulation in Agriculture}
\author{Jarad Niemi}
...
Consider a computer model $f(\cdot)$ with 
\[ 
...
where 
\begin{itemize}
\item $\psi$ are model parameters, \pause
\item $X_i$ are inputs, \pause and
\item $Y_i$ are outputs. \pause 
\end{itemize}

We assume a Gaussian Process prior 
\[ f \sim \mathcal{GP}\left(0, k \right)\]
where $k$ is a covariance kernel. 
...
\item Dealing with functional $X_i$
\end{itemize}
\end{frame}
...
&-\frac{1}{2} y^\top [ \tau^2\I+\Sigma(\theta)]^{-1} y
\end{array} \]
...
\[ k(X_i, X_j) = \sigma^2 e^{-\frac{1}{2} D(X_i(t), X_j(t), \omega)} \]
\pause
...
\begin{frame}
  ...
  \begin{itemize}
    \item \url{https://github.com/jarad/NCCC170_2022}
