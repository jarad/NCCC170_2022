...
For observation $i$, we have $Y_i \in \R$ and $X_i = (X_{i1},\ldots,X_{iD})$
\pause
with PCA-rotated inputs $\tilde{X}_i = (\tilde{X}_{i1},\ldots,\tilde{X}_{iD})$.
\pause
